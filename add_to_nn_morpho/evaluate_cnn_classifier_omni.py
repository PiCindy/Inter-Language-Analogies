...
def evaluate_classifier_transfer(nb_analogies, epochs, mode):
    ...
    device = "cpu" # wtf has happened "cuda" if torch.cuda.is_available() else "cpu"

    LANGUAGES_NEW = ["ang", "dan", "deu", "eng", "frr", "gmh", "isl", "nld", "nob", "swe", "est", "fin", "izh", "liv", "vep", "vot", "mhr", "mdf", "myv", "sme"] #['eng', 'ang', 'azg', 'hil', 'tgl', 'ceb']

    # Load Japanese dataset and split it into train and test

    # Load the models
    path_classif_embed = f"models/omni_classification_cnn/classification_CNN_omni_20e.pth"
    
    
    saved = torch.load(path_classif_embed)
    saved_classif = saved['state_dict_classification']
    saved_embed = saved['state_dict_embeddings']
    
    # Load the test data
    dict_data = {}
    with elapsed_timer() as elapsed:
        for language in LANGUAGES_NEW:
            dict_data[language] = Task0Dataset(language=language, mode="dev", word_encoding="char")
    ...
    for l1 in ['omni']:
        ...
        with elapsed_timer() as elapsed:
            ...
            for l2 in LANGUAGES_NEW:

                # Get the right place to look at for the data
                test_dataset = copy(dict_data[l2])

                if mode == 'full':
                    voc = copy(saved)['voc_id']
                    test_dataset.word_voc = copy(saved)['voc']
                    test_dataset.word_voc_id = voc
                else:
                    path_embed = f"new_class_bal/classification_balanced_CNN_{l2}_{epochs}e.pth"
                ...
                embedding_model.load_state_dict(saved_embed)
                embedding_model.eval()

                classification_model = Classification(emb_size=16*5)
                classification_model.load_state_dict(saved_classif)
    ...
    df = pd.DataFrame(dic_of_accuracies, index = LANGUAGES_NEW).transpose()
