...
def evaluate_classifier_transfer(nb_analogies, epochs, mode):
    ...
    LANGUAGES_NEW = ['eng', 'deu', 'swe', 'fin', 'krl', 'ote']#['eng', 'ang', 'azg', 'hil', 'tgl', 'ceb']
    ...
    with elapsed_timer() as elapsed:
        for language in LANGUAGES_NEW:
            dict_models[language] = Task0Dataset(language=language, mode="trn", word_encoding="char")
    ...
    for l1 in LANGUAGES_NEW:

        with elapsed_timer() as elapsed:

            # Get the right place to look at for the models
            train_dataset = copy(dict_models[l1])
            path_classif = f"new_class_bal/classification_balanced_CNN_{l1}_{epochs}e.pth"
            ...
            for l2 in LANGUAGES_NEW:
                ...
                if mode == 'full':
                    path_embed = f"new_class_bal/classification_balanced_CNN_{l1}_{epochs}e.pth"
                ...
                # Get models
                saved_classif = torch.load(path_classif)['state_dict_classification']
                saved_embed = torch.load(path_embed)['state_dict_embeddings']
